...
trait SM_Front
{
    ...
    public static function sm_get_subcategories($cat_id, $table = 'categories')
    {
        $cat_titles[] = $cat_id;
        $cats = DB::table($table)->where('parent_id', $cat_id)->get();
        ...
        if ($cats) {
            foreach ($cats as $cat) {
                $cat_title[] = $cat->title;
                $new = self::sm_get_subcategories($cat->title, $table);
                $cat_titles = array_merge($new, $cat_title);
            }
        }

        return array_unique($cat_titles);
    }
    ...
    public static function category_tree_for_select_cat_id($category_id, $segment = null)
    {
        ...
        $category_data = SM::getProductsubCategories($category_id);
        ...
        if (count($category_data) > 0) {
            ...
            foreach ($category_data as $s_category) {
                ...
                if (count($s_category->products) > 0) {
                    ...
                    $category_data2 = SM::getProductsubCategories($s_category->id);
                    if (count($category_data2) > 0) {
...
//                            }
    ...
    public static function demo_category_tree_for_select_cat_id($category_id, $segment = null)
    {
...
//                }
    ...
    public static function sm_get_service_tags($tags)
    {
        $tag_title = [];
        if (count($tags) > 0) {
            foreach ($tags as $tag) {
...
//                    if (count($setting) > 0) {
    ...
    public static function sm_get_site_loading_logo()
    {
        if (self::$loading_logo) {
            return self::$loading_logo;
        } else {
            self::$loading_logo = self::get_setting_value('loading_logo');
            ...
            return array(
                ...
                "providers" => array(
                    "Facebook" => array(
                        ...
                        "scope" => "public_profile,email, user_about_me, user_birthday, user_hometown, user_website",
                        // optional
                    )
                )
            );
...
// overflow
    ...
    public static function getHomePopularBlog($noOfPost = 3)
    {
        return SM::getCache('sidebar_popular_blog', function () use ($noOfPost) {
            return Blog::where("status", 1)
                ->orderBy("views", "desc")
...
//                    ->get();
    ...
    public static function getSpecialProduct($noOfPost = 10)
    {
    ...
    public static function getBestSaleProduct($noOfProduct = 10)
    {
...
//                ->limit($noOfProduct)
    ...
    public static function getNewestProduct($noOfProduct = 12)
    {
        $newest_product = SM::getCache('newest_product', function () use ($noOfProduct) {
            return DB::table('products')
                ->where('products.status', 1)
                ->groupBy('products.id')
                ->orderBy('id', 'desc')
        ...
        $get_product_collapse = '
                 ...
                 <li class="active">
                     <a data-toggle="tab" class="common_selector_' . $countP . '" data-category_id="' . $category_id . '"
                       data-type="new" href="">New Arrivals</a>
                 </li>
                 <li>
                    <a data-toggle="tab" class="common_selector_' . $countP . '" data-category_id="' . $category_id . '"
                 ...
                 <li>
                   <a data-toggle="tab" class="common_selector_' . $countP . '"
                       ...
                       href="#tab-' . $category_id . '">Best salers</a>
                 </li> 
                <li>
                    <a data-toggle="tab" class="common_selector_' . $countP . '"
                       data-category_id="' . $category_id . '"
    ...
    public static function wishlistHtml($product_id = null)
    {
        ...
        if (Auth::check()) {
            ...
            $wishlis_html .= '<a href="javascript:void(0);" title="Add to my wishlist" class="btn btn-default btn-heart-ex addToWishlist" data-product_id="' . $product_id . '"><i style=" color:' . $color . ' " class="fa fa-heart"></i></a>';
        } else {
            $wishlis_html .= '<a data-toggle="modal" data-target="#loginModal" title="Add to my wishlist" href="javascript:void(0);" class="btn btn-default btn-heart-ex"><i class="fa fa-heart"></i></a>';
        }
        return $wishlis_html;
    }

    public static function wishlistHtml1($product_id = null)
    {
        $wishlis_html1 = '';
        if (Auth::check()) {
            $check_wishlist = Wishlist::where('product_id', $product_id)->where('user_id', Auth::id())->first();
            ...
            $wishlis_html1 .= '<a href="javascript:void(0);" title="Add to my wishlist" class="wishlist addToWishlist" data-product_id="' . $product_id . '"><i style="background:' . $bc . '; color:' . $color . ' " class="fa fa-heart"></i></a>';
        } else {
            $wishlis_html1 .= '<a data-toggle="modal" data-target="#loginModal" title="Add to my wishlist" href="javascript:void(0);" class="wishlist"><i class="fa fa-heart"></i></a>';
        }
        return $wishlis_html1;
    }
    ...
    public static function compareHtml($product_id = null)
    {
        ...
        $compare_html .= '<a title="Add to compare" data-product_id="' . $product_id . '" class="btn btn-default addToCompare btn-heart-ex"
                                           href="javascript:void(0)" style="background:' . $bc . ';"><i style="color:' . $color . ' " class="fa fa-retweet"></i></a>';

        return $compare_html;
    }

    public static function compareHtml1($product_id = null)
    {
        $compare_html = '';
        $compare = Cart::instance('compare')->content()->where('id', $product_id)->first();
        if (!empty($compare)) {
        ...
        $compare_html .= '<a title="Add to compare" data-product_id="' . $product_id . '" class="compare addToCompare"
                                           href="javascript:void(0)"><i style="background:' . $bc . '; color:' . $color . ' " class="fa fa-retweet"></i></a>';
...
//                 ';
            $add_to_cart_button = '<a href="javascript:void(0)" data-product_id="' . $product_id . '"  
                        data-regular_price="' . $regular_price . '"
                        data-sale_price="' . $sale_price . '"
                        class="addToCart add-to-cart-ex' . $addClass . '" title="Add to Cart" class="addToCart btn btn-success"><i class="fa fa-shopping-cart"></i> Add To Cart </a>';
...
//            ->skip(1)
    ...
    {
        $get_currency = Country::OrderByName()->WhereNotNull()->get();
        return $get_currency;
    }
    ...
    public
    static function product_wait_unit($unit_id = null)
    {
        $get_product_unit = Unit::find($unit_id);
        return $get_product_unit;
    }
...
//            $get_currency = Country::find($cooki_val);
//            if (SM::get_setting_value('currency') != $get_currency->id) {
//                $get_currency_price = $get_currency->currency_symbol . ' ' . number_format($currency_price * $get_currency->currency_rate, 2);
//            } else {
//                $get_currency_price = $get_currency->currency_symbol . ' ' . number_format($currency_price, 2);
//            }
//        } else {
...
//        }
    ...
    public
    static function get_currency_value($currency_price = null)
    {
...
//        }
    ...
    {
        $get_currency = Order::find($order_id);
//        $get_currency = DB::table('orders')->where('id', $order_id)->first();
        if (SM::get_setting_value('currency') != $get_currency->baseCurrency) {
            $get_currency_price = $get_currency->currency_symbol . ' ' . number_format($currency_price * $get_currency->currencyRate, 2);
        } else {
            $get_currency_price = $get_currency->currency_symbol . ' ' . number_format($currency_price, 2);
        }
        return $get_currency_price;
    ...
    public
    static function currencyConverter($from, $to_currency, $amount)
    {
...
//        echo json_encode($data);
    ...
    static function product_review($product_id = null)
    {
        ...
        if ($total_reviews > 0 && $count > 0) {
            $view_review = $total_reviews / $count;
            for ($i = 0; $i < 5; ++$i) {
                if ($view_review <= $i) {
                    $o = '-o';
                } else {
    ...
    public
    static function review_view($id = null)
    {
        $total_reviews = Review::Approved()->where('id', $id)->sum('rating');
        $count = Review::Approved()->where('id', $id)->count();
        $get_review_data = '';
        if ($total_reviews > 0 && $count > 0) {
            $view_review = $total_reviews / $count;
            for ($i = 0; $i < 5; ++$i) {
...
//                return Category::all();
                return Category::Published()->orderBy('priority')->where("parent_id", 0)->get();
            });
        } else {
    ...
    public
    static function getProductmainCategories()
    {
        return SM::getCache('categories_have_not_product', function () {
//                return Category::all();
            return Category::Published()->orderBy('priority')->where('parent_id', 0)->get();
    ...
    public
    static function getProductsubCategories($category_id = null)
    {
    ...
    public
    static function getIsFeaturedCategories()
    {
        $get_is_featured = Category::IsFeatured()
            ->Published()
            ->orderBy('priority')
            ->get();
        return $get_is_featured;
    }
...
//                return Tag::where("total_products", ">", 0)->get();
    ...
    public
    static function getProductBrandsHome($isWithHaveNoPtoduct = 1)
    {
        if ($isWithHaveNoPtoduct == 0) {
            return SM::getCache('brands_have_not_product', function () {
                return Brand::Published()
                    ->IsFeatured()
                    ->orderBy('priority')
                    ->get();
            });
...
//    -----------------End product section------------------------
    ...
    static function sm_get_menu($menu)
    {
        ...
        $a_class = (isset($menu['a_class'])) ? $menu['a_class'] : '';
            ...
            foreach ($main_menu as $item) {
                ...
                if ($item['menu_type'] == 'page') {
                    if (isset($item['p_id']) && $item['p_id'] == 0) {
                        ...
                        $nav .= "<$link_wrapper class='$n_class $cls $hasDropDownWrapperClass $home_class'>\n";
                        $nav .= "<a class='$a_class' href='" . $item['link'] . "' class='$link_cls'>\n";
                        if ($icon_cls != '') {
                            $nav .= "<i class='fa fa-plus-square'></i>\n";
                        }
                        $nav .= ucwords(strtolower($item['title'])) . "\n";
                        $nav .= "<i class='fa fa-sort-desc' aria-hidden='true'></i></a>\n";
                        if ($child != '') {
                    ...
                    if (isset($item['p_id']) && $item['p_id'] == 0) {
                        ...
                        $hasDropDownWrapperClass = ($child != '') ? $has_dropdown_wrapper_class : "";
                        $nav .= "<$link_wrapper class=' $n_class $cls $hasDropDownWrapperClass $home_class'>\n";
                        $nav .= "<a class='$a_class'  href='" . $item['link'] . "' class='$link_cls dropdown-toggle' >\n";

                        $nav .= ucwords(strtolower($item['title'])) . "\n";
                        if ($icon_cls != '') {
                            $nav .= " <i class='fa fa-sort-desc' aria-hidden='true'></i>\n";
                        }
                        if ($child != '') {
                            $nav .= " <i class='fa fa-sort-desc' aria-hidden='true'></i>\n";
                        }
                        ...
                        if ($child != '') {
                            ...
                            $nav .= "\n\n\n<$nav_wrapper class='$subnavClass $dp_class dropdown-header'>";
//                            $nav .= "<$link_wrapper class='dropdown-item'>\n";
//                            $nav .= "\n\n\n<$nav_wrapper class='block'>";
//                            $nav .= "<$link_wrapper class='dropdown-item'><a href='" . $item['link'] . "'>" . ucwords(strtolower($item['title'])) . '</a>';
//                            $nav .= "</$link_wrapper>\n";
                            $nav .= $child;
//                            $nav .= "</$nav_wrapper>\n\n\n";
//                            $nav .= "</$link_wrapper>\n";
...
//        }
    ...
    public
    static function get_Blog_data($noOfProduct = 3)
    {
        $blog = Blog::all();
